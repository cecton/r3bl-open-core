...
// cspell:words LINESIZE getconf DCACHE
...
///    [`Flat2DArray::new_empty`]) to instantly blast empty spacer characters across
///    memory.
...
///    characters left/right during text insertion.
/// 3. **[`Iterator::zip`] ([SIMD] [`memcmp`])**: During the diffing phase, [LLVM]
///    vectorizes the equality checks of two contiguous slices, allowing it to rapidly
///    jump over massive blocks of identical characters.
///
...
///
/// 1. **If you DON'T care about 2D coordinates** (e.g., just clearing the whole screen,
///    or finding the first occurrence of a character): You don't even need chunks. Just
///    blast through the entire raw 1D slice ([`.iter()`], [`.fill()`], etc.). [LLVM] will
///    aggressively vectorize this into [SIMD] instructions because it's just one massive,
///    uninterrupted block of memory.
///
/// 2. **If you DO care about 2D coordinates** (e.g., diffing rows, tracking `col_index`,
///    or knowing when a line ends): [`.chunks_exact(width)`] is the key.
///
///    - **The Math Pipeline Stall Problem (The Slow Way)**. The naive approach uses
///      division (`/`) and modulo (`%`) to calculate coordinates from a 1D index (e.g.,
///      `row = index / width`, `col = index % width`). In the computer world, division
///      and modulo are extremely slow mathematical operations. If your width was a fixed
///      number like 8 or 16 (powers of 2), the compiler could use a lightning-fast
///      bitshift. But because terminal widths vary at runtime (e.g., 113 columns), the
///      compiler cannot optimize this. The CPU is forced to stop and wait for the math to
///      finish for every single character on the screen, causing massive CPU pipeline
///      stalls.
///
...
///      division. Instead, it uses simple **pointer addition**. For example, if the
///      terminal is 113 columns wide, the outer loop just adds 113 to the memory pointer
///      for each row. The inner loop adds 1 to the pointer for each column. Because
///      addition takes 1 clock cycle (unlike division, which takes many), 113 becomes
///      just as fast for the CPU to process as a power of 2 like 128. You get logical 2D
///      row boundaries to write easy-to-understand code, while the [LLVM] compiler sees a
///      predictable contiguous memory layout. It will happily unroll that double loop and
///      vectorize the inner slice comparisons into [SIMD] instructions—all while
///      completely bypassing the slow CPU pipeline stalls.
///
