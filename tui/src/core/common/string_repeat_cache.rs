...
use crate::{DeadlockPreventionPolicy::OptOut, MutexExt, ScopedMutex};
use std::{borrow::Cow,
          collections::HashMap,
          sync::{LazyLock, Mutex}};
...
///
/// ### Thread Safety: [`ScopedMutex<_, DeadlockPreventionPolicy::OptOut>`]
/// - Multiple threads may render simultaneously
/// - [`ScopedMutex`] ensures safe concurrent access using the **Scoped Access** pattern
/// - This cache is in the rendering hot path, so it opts-out of recursion detection by
///   using the [`OptOut`] **coordinate**. See the [Parameters] section in [`ScopedMutex`]
///   for more info.
///
/// ### Memory Management
...
> = LazyLock::new(|| Mutex::new(HashMap::new()).into_scoped_mutex());
